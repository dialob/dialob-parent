...
public class QuestionnairesRestServiceControllerTest {
  ...
  public void shouldPostNewQuestionnaire() throws Exception {
    ...
    when(builder.setCreateOnly(true)).thenReturn(builder);
    when(builder.setFormId(anyString())).thenReturn(builder);
    when(builder.setFormRev(any())).thenReturn(builder);
    when(builder.setSubmitUrl(any())).thenReturn(builder);
    when(builder.setContextValues(any())).thenReturn(builder);
    when(builder.setAnswers(any())).thenReturn(builder);
    when(builder.setValueSets(any())).thenReturn(builder);
    when(builder.setLanguage(any())).thenReturn(builder);
    when(builder.setCreator(any())).thenReturn(builder);
    when(builder.setOwner(any())).thenReturn(builder);
    when(builder.setStatus(any())).thenReturn(builder);
    when(builder.setActiveItem(any())).thenReturn(builder);
    when(builder.setAdditionalProperties(any())).thenReturn(builder);
    ...
    verify(builder).setCreateOnly(true);
    verify(builder).setFormId("new-form");
    verify(builder).setFormRev(null);
    verify(builder).setCreator(null);
    verify(builder).setOwner(null);
    verify(builder).setSubmitUrl(null);
    verify(builder).setContextValues(anyList());
    verify(builder).setAnswers(anyList());
    verify(builder).setValueSets(anyList());
    verify(builder).setLanguage(null);
    verify(builder).setStatus(Questionnaire.Metadata.Status.NEW);
    verify(builder).setActiveItem("page3");
    verify(builder).setAdditionalProperties(any());
