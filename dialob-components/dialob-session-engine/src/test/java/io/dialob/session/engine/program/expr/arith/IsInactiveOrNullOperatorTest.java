...
class IsInactiveOrNullOperatorTest {

  @Test
  void shouldReturnBooleanType() {
    IsInactiveOrNullOperator operator = IsInactiveOrNullOperator.of(IdUtils.toId("item1"));
  ...
  void shouldReturnEvalRequiredConditions() {
    ...
    IsInactiveOrNullOperator operator = IsInactiveOrNullOperator.of(itemId);
    Set<EventMatcher> conditions = operator.getEvalRequiredConditions();
    assertEquals(Set.of(whenValueUpdated(itemId), whenActiveUpdated(itemId)), conditions);
  }

  @Test
  void shouldEvalToTrueWhenItemIsInactive() {
    ItemId itemId = IdUtils.toId("item1");
    IsInactiveOrNullOperator operator = IsInactiveOrNullOperator.of(itemId);
    ...
    when(itemState.getValue()).thenReturn("someValue");

    assertTrue(operator.eval(context));
  }

  @Test
  void shouldEvalToTrueWhenItemValueIsNull() {
    ItemId itemId = IdUtils.toId("item1");
    IsInactiveOrNullOperator operator = IsInactiveOrNullOperator.of(itemId);
    EvalContext context = Mockito.mock(EvalContext.class);
    ItemState itemState = Mockito.mock(ItemState.class);
    ...
    when(itemState.getValue()).thenReturn(null);

    assertTrue(operator.eval(context));
  }

  @Test
  void shouldEvalToFalseWhenItemIsActiveAndValueIsNotNull() {
    ItemId itemId = IdUtils.toId("item1");
    IsInactiveOrNullOperator operator = IsInactiveOrNullOperator.of(itemId);
    EvalContext context = Mockito.mock(EvalContext.class);
    ItemState itemState = Mockito.mock(ItemState.class);
