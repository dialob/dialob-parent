import { SessionItem, SessionError, Session, ItemType } from '@dialob/fill-api';
...
export function useFillItem<T extends ItemType>(id: string | undefined): FillItem<T> {
  ...
  const [item, setItem] = useState<SessionItem<T> | undefined>(() => id ? session.getItem(id) as SessionItem<T> | undefined : undefined);
