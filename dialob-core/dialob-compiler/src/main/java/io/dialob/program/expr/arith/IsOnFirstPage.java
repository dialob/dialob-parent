...
@Value.Immutable
public interface IsOnFirstPage extends Expression {

  @Override
  default Boolean eval(@Nonnull EvalContext context) {
    return context.getItemState(DialobSession.QUESTIONNAIRE_REF).map(questionnaire -> questionnaire.getActivePage().map(itemRef -> {
