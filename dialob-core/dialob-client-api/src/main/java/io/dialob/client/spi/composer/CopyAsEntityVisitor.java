...
public class CopyAsEntityVisitor {
  ...
  public CopyAsEntityVisitor(ComposerState state, String assetId, String copyToName, DialobClient client) {
    super();
    this.asset = asset;
    this.state = state;
  ...
  private void visitValidations() {
    ...
    final var decision = state.getDecisions().values().stream()
      ...
      .filter(e -> e.getDocument().getName().equals(asset.getName()))
      .findFirst();
  ...
  public List<AstCommand> initFlow(ComposerEntity<?> original) {
    ...
    final var body = finalCommand.getData();
    final var oldName = original.getDocument().getName();
