...
import {Table, Segment, Label, Icon, Button, Input, Form} from 'semantic-ui-react';
import {connect} from 'react-redux';
import {createValidation, deleteValidation, updateValidation} from '../actions';
import * as Defaults from '../defaults';
import { METHODS } from 'http';

const ValidationRule = ({onChangeMessage, onChangeRule, onRemove, message, rule}) => {
  return (
    <Segment>
      ...
      <Form>
        ...
        <Form.Input label='Validation rule' icon='check' fluid value={rule || ''} onChange={onChangeRule} />
      </Form>
    </Segment>
   );
};
